...
pub fn apply_thumbnail(image: &gtk::Image, file: &gio::File, info: &gio::FileInfo, size: i32) {
    ...
    thread::spawn(move || {
        let thumb_path = ensure_thumbnail_file(&path, &uri, &content_type, gen_size);
        glib::MainContext::default().invoke(move || {
            ...
            if let Ok(pixbuf) = Pixbuf::from_file_at_scale(&thumb_path, size, size, true) {
