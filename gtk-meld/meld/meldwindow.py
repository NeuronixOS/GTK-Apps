...
def _load_gtk_theme():
    """Import shared suite gtk_theme (source tree or installed sibling)."""
    try:
        import gtk_theme
...
class MeldWindow(Adw.ApplicationWindow):
    ...
    def _setup_gtk_theme(self) -> None:
        """Fold suite Profile themes into the existing gear menu (View)."""
        try:
            gtk_theme = _load_gtk_theme()
        except ImportError:
            log.warning("gtk-theme not available; suite Profile menu disabled")
            return

        menu = self.gear_menu_button.get_menu_model()
        if menu is None:
            log.warning("gear menu missing; suite Profile menu disabled")
            return

        gtk_theme.integrate_profile_menu(
            self,
            menu,
            section_id="profile-section",
            before_section_id="application-section",
        )
        # Keep SourceView + diff chunk colours aligned with the suite Profile.
        self._apply_suite_profile(gtk_theme.load_profile())
        gtk_theme.watch_theme(self._apply_suite_profile)

    def _apply_suite_profile(self, profile) -> None:
        """Apply suite Profile to Adw, syntax scheme, and diff chunk colours."""
        try:
            gtk_theme = _load_gtk_theme()
        except ImportError:
            return
        ...
        import meld.style
        from meld.settings import get_meld_settings, settings as gsettings

        # Chunk colours first — must not be skipped if gsettings calls fail.
        try:
            fill_hex, line_hex = gtk_theme.meld_chunk_colours(profile)
            meld.style.set_suite_chunk_colours(fill_hex, line_hex)
        except Exception:
            log.exception("Failed to apply suite diff chunk colours")

        try:
            gtk_theme.apply_adw_color_scheme(profile)
        except Exception:
            log.exception("Failed to apply Adw color scheme")

        try:
            # Force light/dark to match profile (Adw enum values match gschema).
            variant = 4 if profile.is_dark() else 1  # force-dark / force-light
            if gsettings.get_enum("style-variant") != variant:
                gsettings.set_enum("style-variant", variant)

            mgr = GtkSource.StyleSchemeManager.get_default()
            scheme_id = gtk_theme.resolve_sourceview_scheme(
                profile.id, lambda sid: mgr.get_scheme(sid) is not None
            )
            if gsettings.get_string("style-scheme") != scheme_id:
                gsettings.set_string("style-scheme", scheme_id)
        except Exception:
            log.exception("Failed to sync style-scheme / style-variant")

        # Refresh open views (buffers + gutters) even if scheme id was unchanged.
        try:
            get_meld_settings().emit("changed", "style-scheme")
        except Exception:
            log.exception("Failed to refresh meld style-scheme listeners")
    ...
    def do_realize(self):
        ...
        # Re-apply after realize so first comparison picks up suite colours.
        try:
            gtk_theme = _load_gtk_theme()
            self._apply_suite_profile(gtk_theme.load_profile())
        except ImportError:
